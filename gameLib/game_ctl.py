...
class GameControl():
    ...
    def wait_game_img(self, img_path, max_time=100, quit=True):
        ...
        while time.time() - start_time <= max_time and self.run:
            ...
            if max_time > 5:
                ...
                time.sleep(0.1)
        # 超时前做下拒绝悬赏 如果是因为悬赏而超时则在检测一遍 如果正常则继续否则进行脚本超时处理
        if self.rejectbounty():
            maxVal, maxLoc = self.find_img(img_path)
            if maxVal > 0.9:
                return maxLoc
        if quit:
    ...
    def rejectbounty(self):
        '''
        拒绝悬赏或接收勾协
            :return: 拒绝成功接收勾协返回True，其他情况返回False
        '''
        ...
        if maxVal > 0.9:
            ...
            if not self.magatama_reject:
                ...
                maxVal, maxLoc = self.find_img('img\\XuanShangFengYinXie.png')
                if maxVal > 0.9:
                    logging.info("判定为勾协接收勾协")
                    self.mouse_click_bg((random.randint(740, 770), random.randint(355, 385)))
                    return True
            logging.info("拒绝悬赏封印")
            self.mouse_click_bg((random.randint(740, 770), random.randint(443, 475)))
            return True
        return False
